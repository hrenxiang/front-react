...
function installHook(target) {
  ...
  function inject(renderer) {
    ...
    // This provides React Native developers with pages stacks even if they don't run DevTools.
...
function attach(hook, rendererID, renderer, global) {
  ...
  function didFiberRender(prevFiber, nextFiber) {
    switch (nextFiber.tag) {
      ...
      default:
        // For host pages and other types, we compare inputs
  ...
  function updateFiberRecursively(nextFiber, prevFiber, parentFiber, traceNearestHostComponentUpdate) {
    ...
    // Suspense pages only have a non-null memoizedState if they're timed-out.
  ...
  // Could use inspectElementRaw() but that would require shallow rendering hooks pages,
  ...
  function inspectElementRaw(id) {
    ...
    if (tag === ClassComponent || tag === FunctionComponent || tag === IncompleteClassComponent || tag === IndeterminateComponent || tag === MemoComponent || tag === ForwardRef || tag === SimpleMemoComponent) {
      ...
      if (stateNode && stateNode.context != null) {
        // Don't show an empty context object for class pages that don't use the context API.
  ...
  function inspectElement(id, path) {
    ...
    if (isCurrent) {
      ...
      // (Reducing how often we shallow-render is a better DX for function pages that use hooks.)
      ...
      switch (type) {
        case 'context':
          ...
          switch (fiber.tag) {
            ...
            case FunctionComponent:
              // Function pages using legacy context are not editable
  ...
  function getDisplayNameForRoot(fiber) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (displayName !== null) {
        // Prefer display names that we get from user-defined pages.
        // We want to avoid using e.g. 'Suspense' unless we find nothing else.
        if (typeof child.type === 'function') {
          // There's a few user-defined tags, but we'll prefer the ones
          // that are usually explicitly named (function or class pages).
